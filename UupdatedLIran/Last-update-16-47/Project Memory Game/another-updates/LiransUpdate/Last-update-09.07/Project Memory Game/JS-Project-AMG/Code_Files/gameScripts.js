...
// פונקציית טיימר
    ...
    timerSet = () => {
        if(isTrue){
            ...
            stopTimer();
            isTrue = !isTrue
            document.getElementById('btn_stop_move').value = "start";
...
let livesLeft = document.querySelectorAll('.heartLife')
...
// after clicking 1 card and go to the second card and click on it, this will happend
    ...
    match();                                                                  // function that check if the class of the card is the same as the other card
};
let checkend = 0;
...
function match(){                                                             // function that check if card1 and card2 have the same class
    if (firstCardChosed.className == secondCardChosed.className) {
        ...
        // console.log(points);
        firstCardChosed.addEventListener('click', flipTheCard);               // let it click back
        secondCardChosed.addEventListener('click', flipTheCard);              // let it click back
    }                                                                 
};
function cantClick(){
...
function flipBack (){
    setTimeout(() => {                                                        // setting delay so the card will flip toghether after watching the cards
        ...
        secondCardChosed.classList.remove('flip');                            // remove the class flip from the card ( it means that the card will flip back)
